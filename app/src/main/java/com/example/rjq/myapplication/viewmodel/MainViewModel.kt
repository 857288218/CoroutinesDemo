    ...
    fun login(userName: String, pwd: String): LiveData<WanResponse<User>>? {
        if (userLive == null || (userLive.value == null)) {
            ...
            viewModelScope.launch {
                ...
                //CoroutineScope().launch/async{}是需要等其后面的代码执行完再执行该协程里的代码相当于向该线程postMessage,可以使用Dispatchers.Main.immediate做到立即执行协程块里的代码,viewModelScope/lifecycleScope就使用的Dispatchers.Main.immediate
